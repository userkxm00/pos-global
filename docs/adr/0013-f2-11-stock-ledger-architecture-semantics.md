...
Status: Accepted — Approved for Planning & Implementation
Date: 2026-09-08
...
Following milestone `F2.10 — Locations / Bins` (`019_locations_bins.sql`), which established physical topography master data (`locations` and `bins`) with zero quantity tracking, milestone `F2.11 — Stock Ledger` operationalizes spatial inventory tracking.

In retail, wholesale, and multi-facility commercial operations:
- **At the aggregate level**, inventory balances track total physical ownership per product and variant within a branch (`inventory.quantity_milli`).
- **At the spatial level**, inventory must be physically located in specific macroscopic zones (`locations`) and addressable storage slots (`bins`), tracked in `location_inventory`.
...
- **At the lot and serial level**, lots (`product_batches`) and uniquely identified assets (`serial_numbers`) must link seamlessly to physical slots without data fragmentation or dual-source-of-truth conflicts.
...
2. **Batch Lot Source of Truth (`016_batches_and_expiry.sql:32`):**
...
5. **Decision D5 — Formal Milestone Partitioning:**
   - Intra-branch zero-net relocations belong strictly to **F2.12 Transfers**.
   - Shrinkage taxonomies, loss classification hierarchies, and approval workflows belong strictly to **F2.13 Adjustments**. F2.11 provides only the primitive movement reason `adjustment`.
...
1. **Branch Aggregate Inventory (`inventory`):**
   Source of truth for total physical inventory owned by the branch for a product/variant.
...
The system enforces non-negative stock fail-closed:
...
- `opening_balance` does **NOT** mean *"automatically allocate historical inventory during migration"*.
...
1. **Legacy Serials (Pre-020):**
   ...
   - Deducting a legacy serial lacking coordinates fails with `StockLedgerError::SerialCoordinateMismatch("Serial has no physical location assigned")`.
   - Physical location attribution belongs to **F2.14 (Stock Count & Physical Reconciliation)**, while point-of-sale customer checkout belongs to **Phase 3 (F3.03 Sales Checkout)**.

2. **Serial Creation vs. Stock Intake Boundary (Reconciliation with F2.08):**
   ...
   - Dedicated product recall workflows are deferred to future business disposition milestones rather than bypassing the ledger authority through unmanaged status mutations.

3. **Serial Deduction & Adjustment Semantics:**
   - When a serialized unit is deducted from stock ($\Delta_{\text{milli}} = -1000$):
     ...
     - `MovementReason::Loss`: Maps serial to `disposed` and clears coordinates (`location_id = NULL, bin_id = NULL`). Status `disposed` is a permanent terminal state.
     - `MovementReason::Adjustment`: Maps serial to `reserved` and clears coordinates (`location_id = NULL, bin_id = NULL`). Here, `reserved` serves as the non-stock holding state.
   - A subsequent positive adjustment (`MovementReason::Adjustment` with $\Delta_{\text{milli}} = +1000$) specifying valid `location_id` and optional `bin_id` transitions a `reserved` serial back to `in_stock`, reinstating physical coordinates and incrementing spatial and aggregate balances with a new immutable movement record.
   - Similarly, a repaired unit in `defective` status may transition back to `in_stock` via a positive adjustment with valid physical coordinates, as authorized by F2.08 lifecycle rules.

4. **Batch Creation vs. Stock Intake Boundary (Reconciliation with F2.07):**
   - Under F2.07 (`batch::create_batch`), product batches are registered as master lot identity with initial `quantity_milli = 0` and initial status `depleted`. Direct creation with positive quantity is rejected fail-closed.
   - Initial on-hand batch stock enters **exclusively** through `StockLedgerService::post_movement` with `batch_id`, valid `location_id`, and $\Delta > 0$, transitioning the batch from `depleted` to `active` while atomically updating spatial and aggregate balances.
...
1. `opening_balance`: Initializing physical stock in a location/bin ($\Delta > 0$).
2. `adjustment`: Modifying physical stock in a location/bin ($\Delta \ne 0$).
3. `damage`: Writing off damaged stock from a location/bin ($\Delta < 0$).
4. `loss`: Writing off lost/stolen stock from a location/bin ($\Delta < 0$).
...
- Any movement request with empty or missing `location_id` is rejected immediately with `StockLedgerError::MissingLocation`.

### 7.4 Atomic Mutation Sequence (Single SQLite Transaction)
1. **Begin Transaction:** `let tx = conn.transaction()?;`
2. **Idempotency Gate (INSIDE transaction):**
   - Query `idempotency_keys` for `req.idempotency_key`.
   - If found and `request_hash == req.canonical_hash()`: return cached `StockMovementResult` (transaction rolls back cleanly, zero duplicate mutations).
   - If found and `request_hash != req.canonical_hash()`: return `Err(StockLedgerError::IdempotencyConflict)` (transaction rolls back cleanly).
3. **Validate Invariants:**
   ...
   - Verify `product_id` is active; verify `variant_id` belongs to product and is active.
   - If batch: verify batch belongs to product, branch, and variant; check non-negative batch balance; use `checked_add` for arithmetic.
   - If serial: verify serial belongs to product, branch, and variant; check status eligibility; check coordinate consistency; enforce $|\Delta_{\text{milli}}| = 1000$.
4. **Mutate Balances:**
   - Update `inventory.quantity_milli += delta` (fail-closed if $< 0$).
   - Upsert `location_inventory.quantity_milli += delta` (fail-closed if $< 0$).
...
### Completeness of Canonical Field Set
- `branch_id`, `product_id`, `variant_id`: identifies the catalog entity grain.
- `location_id`, `bin_id`: identifies the spatial storage slot grain.
- `batch_id`, `serial_id`: identifies the discrete lot or asset instance.
- `quantity_delta_milli`: identifies the physical quantity magnitude and sign.
- `reason`: identifies the operational movement classification.
Any difference in any of these fields represents a materially distinct inventory operation. If presented under an existing idempotency key, it will be rejected as an `IdempotencyConflict`.

---

## 9. Historical Reconstruction & Audit Streams

The append-only ledger `stock_movements` provides four distinct audit streams:

1. **Attributed Spatial History:**
   All spatial slot balances are strictly reconstructable across the `location_inventory` grain:
   $$\text{location\_inventory.quantity\_milli}(br, loc, bin, prod, var, bat) = \sum \text{quantity\_delta\_milli}$$
2. **Aggregate Branch History:**
   Reflects the pre-020 legacy unallocated baseline snapshot at Migration 020 cutoff timestamp $T_{\text{cutoff}}$ plus all post-020 recorded movement deltas:
   $$\text{inventory.quantity\_milli} = \text{legacy\_baseline} + \sum_{t \ge T_{\text{cutoff}}} \text{quantity\_delta\_milli}$$
3. **Legacy Unattributed History:**
   Historical movement rows with `location_id IS NULL` document historical volume changes without retroactive spatial attribution.
...
-- 4. Prerequisite unique indexes for composite foreign keys
...
### 10.2 Exactly 8 Mutually Exclusive Partial Unique Indexes
...
| **F2.13 Adjustments** | Deferred to F2.13. | Owns shrinkage taxonomies, adjustment approval workflows, and loss classification hierarchies. F2.11 provides only the primitive movement reason `adjustment`. |
